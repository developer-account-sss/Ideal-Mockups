...
const CostEstimation = () => {
    ...
    async function fetchCostAndTime() {
      try {
        const token = localStorage.getItem('token');
          const response = await axios.get('http://127.0.0.1:5000/price_factor',{
            headers: {
  ...
  const fetchData = () => {
    const token = localStorage.getItem('token');
    axios.get('http://127.0.0.1:5000/api/yourData',{
      headers: {
        'Authorization': `Bearer ${token}`
      }
    })
  ...
  const fetchClients = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('http://127.0.0.1:5000/get_clients',{
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleAddClient = async () => {
    try {
      const token = localStorage.getItem('token');
      console.log(token)
        await axios.post('http://127.0.0.1:5000/add_client', {
           name: newClientName
        },
          {headers: {
            'Authorization': `Bearer ${token}`,
  ...
  const handleDelete = (id) => {
    const token = localStorage.getItem('token');
    axios.delete('http://127.0.0.1:5000/api/yourData', {
      headers: {
        'Authorization': `Bearer ${token}`
      }, data: { id }
    })
  ...
  useEffect(() => {
    ...
    const predictToolDia = async () => {
      try {
        ...
        // Send a POST request to the Flask API
        const token = localStorage.getItem('token');
        const response = await axios.post('http://127.0.0.1:5000/predictToolDia', requestToolDia, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
  ...
  const handleCalculate = () => {
    ...
    if (selectedRowId){
      const token = localStorage.getItem('token');
      axios.patch('http://127.0.0.1:5000/api/yourData', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      }, mergedData)
      ...
      const token = localStorage.getItem('token');
      axios.post('http://127.0.0.1:5000/api/yourData', postData,
        {headers: {
          'Authorization': `Bearer ${token}`
        }
      })
  ...
  // const requiredFields= ['date',  'mockupNo',  'pltNO',  'partNo',  'commonDesc',  'description',  'client',  'category',  'absAcrylic',  'color',  'finish',  'printCr',  'light',  'length',  'width',  'height',  'blockHeight',  'vol',  'area',  'stockOptimize',  'millingOptimize',  'detailingLevel',  'printAreaOptimize',  'toolDiaA'];
  ...
  const columnDefs = [
    ...
    { headerName: 'ESTIMATE NO', field: 'pltNO', rowGroup: true, hide: true},
            ...
            <div className='d-flex justify-content-between gap-3' style={{overflow:"scroll"}}> 
              <div>
                <div className='d-flex justify-content-between'>
                  ...
                  <FormControl sx={{ minWidth: 210 }} size="small">
                      <InputLabel  required="true" id="absAcrylic-label">ABS / ACRYLIC</InputLabel>
                      <Select
